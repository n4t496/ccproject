...
cluster = Cluster(['35.246.22.174'])
session = cluster.connect('weather10')
...
def weatherinfo():
    ...
    rows = session.execute( """select city_state from stats where city= '{}' ALLOW FILTERING""".format(my_city))

    for weather10 in rows:
        city_stateid = weather10.city_state

    weather_url = weather_url_template.format(key = my_api_key, city = city_stateid, start = start_date, end = end_date)
